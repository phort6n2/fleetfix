...
/* Shared FAQ entries reused where they apply. Each page still gets a
...
/* ======================================================================= */
{
  ...
  desc: 'Mobile windshield replacement and auto glass repair across Colorado’s Front Range. Fleet programs, truck and equipment glass, insurance billed direct.',
...
<p>Most auto glass shops are set up for one car at a time and treat commercial work as an occasional extra. FleetFix runs the other way around. The scheduling, the billing and the van stock are all organized for operators: fleets, contractors, haulers and trades. That is how we can put a technician in your yard and work through six vehicles in an afternoon instead of booking six separate appointments.</p>
<p>We have still never turned away a single cracked windshield. The same technician who spends the morning on a distribution fleet spends the afternoon in someone's driveway in Wheat Ridge, and the work is done to the same standard either way.</p>
...
<p>Where a vehicle has a forward-facing camera mounted to the windshield, which now covers most vehicles built in the last several years, the camera has to be recalibrated after the glass is replaced. We handle that as part of the job instead of sending you somewhere else to finish it.</p>
...
<div class="callout">
<p><b>If you have a chip, deal with it early.</b> A chip that can be filled in half an hour becomes a full replacement once the crack reaches the edge of the glass or crosses the driver's line of sight. Repairs are also much cheaper, and many insurers waive the deductible on a repair specifically because it saves them a replacement later.</p>
</div>
...
<p>Those are ranges, not quotes. Tell us the year, make and model, or better still the VIN, and we will come back with a firm number that includes the mobile visit, the glass, the fitting and any calibration the vehicle needs. What we quote is what you pay.</p>
`,
  faq: [
    { q: 'Do you charge extra to come to me?', a: 'No. Mobile service is how we work, not an add-on, and there is no travel charge anywhere inside our normal service area, which runs from Greeley in the north to Castle Rock in the south, and across the metro from Golden to Aurora. If you are outside that, call and ask; we will either quote the trip or tell you that someone closer makes more sense.' },
    FAQ_INSURANCE,
    { q: 'How long does a replacement take?', a: 'Around 45 to 60 minutes of fitting time for a typical car or light pickup, and 60 to 90 minutes for larger commercial vehicles. The part that costs people time is the adhesive cure. The urethane needs time to reach the strength where the windshield can do its structural job in a crash. Your technician will give you a specific safe drive-away time on the day, because it depends on the adhesive used and the temperature it cured at.' },
    { q: 'What vehicles do you work on?', a: 'Passenger cars, pickups, vans, box trucks, semi tractors, buses, motorhomes, and construction plant such as excavators, loaders, skid steers and backhoes. Farm equipment cab glass too. The awkward ones are usually a sourcing question, not a fitting one, so the sooner we have the VIN or the serial plate the sooner we can confirm a date.' },
    { q: 'Do you offer fleet accounts?', a: 'Yes, and it is most of what we do. An account gets you priority scheduling, pricing based on volume instead of per-job, purchase-order invoicing, net-30 terms once the account is approved, and a service record against each unit so you can see what has been done to which vehicle. It works the same whether you run five vans or several hundred.' },
...
<p>This is not a cosmetic call. A bonded windshield is a structural part of the vehicle: it contributes to roof crush resistance in a rollover and it provides the backboard the passenger airbag inflates against. A compromised windshield is a safety problem, not a visibility one.</p>
...
`,
  faq: [
    { q: 'Can you replace glass outdoors in winter?', a: 'Usually, yes. We carry cold-weather urethane rated for low-temperature application and we work under cover where we can. What we will not do is fit glass in conditions that stop the adhesive curing properly: driving snow, or temperatures below what the product is rated for. If that is the situation on the day we will say so and rebook instead of handing you a windshield that is not bonded correctly.' },
    { q: 'Is OEM-equivalent glass as safe as manufacturer glass?', a: 'Yes, in the sense that matters: both have to meet the same federal safety standard for automotive glazing, and both are laminated safety glass built to that standard. They are not identical: interlayer thickness, acoustic layers and bracket detail can differ, and that is why we match to the VIN. The genuine article carries the vehicle maker’s branding and is built on their tooling, which occasionally gives a closer fit on optical distortion near the edges. For most vehicles the equivalent is what your insurer will approve and what we would fit to our own vehicles.' },
    ...
    FAQ_INSURANCE,
    { q: 'What does the warranty actually cover?', a: 'Our workmanship, for as long as you own the vehicle. If the installation leaks, whistles, or the seal fails, we come back and put it right at no cost. It does not cover new impact damage; a fresh rock is a fresh rock. Nor does it cover a defect in the glass itself, which is handled under the manufacturer’s warranty.' },
  ],
},

{
  ...
  ogTitle: 'Windshield Chip & Crack Repair | Front Range',
...
<p>The glass your vehicle left the factory with was bonded in a controlled environment by robots, and that original seal is difficult to improve on. Every replacement, however well done, means cutting that bond out and creating a new one. If the damage can be repaired instead, you keep the factory seal, you keep the original glass, and you avoid recalibrating any camera mounted to it.</p>
...
<div class="callout">
<p><b>Time is the enemy.</b> Once water gets into the break, freezes overnight and expands, the break spreads. Colorado's overnight temperature swings do this reliably. The gap between "half-hour repair" and "full replacement" is often a single cold night.</p>
</div>
...
<p>That is the design brief for our fleet program: everything about it aims at compressing downtime, not at shaving a few dollars off the glass.</p>

<h2>How a fleet account works</h2>
<h3>Service days instead of appointments</h3>
<p>Rather than booking vehicles in one at a time, we schedule a block at your yard and work through the list. A technician arrives with the glass for every unit already sourced against its VIN, and units get handled as they become free, so a driver returning at eleven does not lose their slot because they were not there at nine.</p>
...
<p>We invoice against your PO numbers, reference your unit numbers on every line, and offer net-30 terms once the account is approved. Statements can be consolidated monthly instead of arriving per job, which makes reconciliation something one person does once instead of something accounts payable does forty times.</p>
...
`,
  faq: [
    ...
    { q: 'Can you invoice against our purchase orders?', a: 'Yes. We reference your PO number and your internal unit number on every line item, so the invoice reconciles against your system, not ours. Consolidated monthly statements are available if you would rather not process individual invoices.' },
    { q: 'Do you handle commercial insurance claims?', a: 'Yes, both commercial policies and personal ones. For fleet claims we deal with your carrier or your broker directly and bill them, and we will work to whatever claim-reporting process you already have in place instead of asking you to adopt ours.' },
...

{
  ...
  body: `
<h2>Work trucks are their own job</h2>
<p>A three-quarter-ton pickup with a ladder rack and a utility body is a different proposition from a sedan, and pretending otherwise is how glass gets damaged during fitting. The windshields are larger and heavier, which frequently makes it a two-person set, not a one-person one. The A-pillar geometry is different. And there is usually something bolted near the glass, whether a rack upright, a light bar, a beacon or an antenna mount, that has to come off carefully and go back on correctly.</p>
<p>We do this daily, so the awkward parts are planned for, not discovered on site. If your truck has an upfit, tell us what it is when you call and the technician arrives with the right hands and the right kit.</p>
...
<p>Box truck cabs deserve a specific mention: the glass is often a flat or lightly curved unit sitting in a rubber gasket instead of bonded in urethane, which is a different fitting method. Both are routine for us, but they are not interchangeable skills, and a shop that only does passenger cars will struggle with the gasket-set ones.</p>

<h2>Getting the right glass first time</h2>
<p>On a modern truck, trim level decides the windshield. Within a single model year you might be choosing between glass with or without a rain sensor, with or without a heated wiper park area, with or without an acoustic layer, and with or without a bracket for a forward camera. Guess wrong and the technician arrives with a windshield that physically fits but leaves a sensor unmounted.</p>
<p>The VIN settles all of it. Give us the VIN when you book and we will confirm the exact part before the van loads.</p>

<div class="callout">
<p><b>Heavy-duty pickups increasingly have cameras.</b> Lane keeping and adaptive cruise are now common well up the weight range, and any of those systems means recalibration after replacement. We check this against the VIN at quote stage so it is priced in from the start instead of appearing as an extra afterwards.</p>
</div>

<h2>Downtime, and how we keep it short</h2>
<p>Fitting a truck windshield takes roughly 60 to 90 minutes depending on the vehicle and what has to come off first. Add the adhesive cure before the truck is safe to drive, and you are looking at a couple of hours out of a working day, not a whole one.</p>
...

{
  ...
  desc: 'Mobile semi truck windshield replacement across Colorado’s Front Range: Freightliner, Peterbilt, Kenworth, Volvo and International, at your terminal.',
  ...
  heroPoints: [
    ...
    'We come to the terminal, so no deadheading a tractor to a glass shop',
...
<p>So we treat semi glass as urgent work by default. A cracked tractor windshield is not something to schedule for next week.</p>
...
<p>Freightliner Cascadia, Columbia, Coronado and M2. Peterbilt 379, 389, 567, 579 and 587. Kenworth W900, T680, T800 and T880. Volvo VNL and VNR. Mack Anthem, Pinnacle and Granite. International LT, ProStar, LoneStar and HX. Western Star 4700, 4900 and 5700. Vocational units are all in scope too: mixers, dumps, refuse and cranes.</p>

<h2>Working at a terminal</h2>
<p>Bringing a tractor to a glass shop costs more than the glass: you are moving an 80,000-pound-capable vehicle, paying a driver to sit through the job, and losing most of a shift. So we come to you: terminals, drop yards, truck stops, distribution centers, or the shoulder of your own lot.</p>
<p>What we need is straightforward: the tractor on level ground, clearance to work across the full width of the cab, and ideally the unit not blocked in by trailers. We bring everything else. Heavy glass is a two-technician job and we staff it that way instead of having one person wrestle a windshield the size of a door.</p>

<div class="callout">
<p><b>Cure time on a tractor is not optional either.</b> The urethane bonding a heavy truck windshield needs the same respect as on a car, and the glass is a great deal heavier. We will give you a safe drive-away time. Dispatching a tractor before it is reached is a real safety risk, not a formality.</p>
</div>
...
`,
  faq: [
    { q: 'Can you replace just one half of a two-piece windshield?', a: 'Yes, and on a split-glass tractor that is usually exactly what we do, because there is no reason to replace undamaged glass. It brings the cost down substantially compared with doing both sides. If the center seal or the post gasket is also perished we will flag it, because replacing the glass and leaving a failed seal behind just produces a leak.' },
...

{
  ...
  desc: 'Mobile auto glass across Denver and Colorado’s Front Range. We replace and repair glass at your home, office, job site or fleet yard at no extra charge.',
...
<p>What we will not do is fit glass in conditions where the bond cannot form properly: heavy driving snow, rain onto an open pinch weld, or temperatures below what the product is rated for. In that situation we will tell you on the day and rebook instead of handing back a vehicle with a windshield that is not structurally bonded. It is inconvenient once; the alternative is a safety problem you cannot see.</p>

<div class="callout">
<p><b>Plan for the cure as well as the fitting.</b> The appointment is 45 to 90 minutes depending on the vehicle, but the windshield is not at full strength the moment the technician steps back. Ask for your safe drive-away time and build it into the day, particularly in winter, when it is longer.</p>
</div>

<h2>Multiple vehicles in one visit</h2>
<p>The economics of mobile work strongly favor doing several vehicles at once, and that is what fleet service days are for. If you have more than a couple of units needing glass, grouping them into one visit is cheaper for you and more efficient for us, and we will price it accordingly.</p>
`,
  faq: [
    { q: 'Is mobile service really free?', a: 'Inside our normal service area, yes. There is no separate travel or call-out charge, and the price we quote for the glass is the price you pay. Outside that area we will quote the trip openly instead of hiding it, or tell you if someone closer to you makes better sense.' },
    ...
    { q: 'What happens if the weather turns on the day?', a: 'We call you. If the conditions will stop the adhesive curing properly we rebook instead of fitting anyway. A windshield that has not bonded correctly is a structural problem, not a cosmetic one. For chip repairs the bar is lower than for a bonded replacement, since the resin cures under UV light instead of depending on ambient conditions, though we still will not fill a break that is wet or frozen, because the resin cannot bond to it.' },
...
<p>Being metro-based means short drives, so Denver ZIP codes tend to get the tightest arrival windows and the best odds on a same-day chip repair.</p>
...
<p>Every load of gravel and aggregate coming down from the high country moves through the metro, and trucks shed material at highway speed. Damage picked up on the westbound climb tends to arrive as a star break, not a tidy bullseye, because the closing speed is higher.</p>
...
`,
  faq: [
    ...
    { q: 'How quickly can someone get to me in Denver?', a: 'Being metro-based, Denver ZIP codes usually get the shortest lead times we offer. Chip repairs can often be fitted in same-day when the schedule allows. Replacements depend mainly on glass availability for your specific vehicle, not on our capacity. Give us the VIN and we can tell you the same day.' },
    { q: 'Do you handle hail damage claims?', a: 'Yes, and we bill the carrier directly. One caveat: after a major hail event across the metro, demand for glass spikes hard and specific windshields can go on backorder nationally. Reporting the claim and getting the vehicle booked early makes a real difference to how long you wait.' },
    { q: 'Is your Sheridan Boulevard location a drop-in shop?', a: 'It is primarily our base for stock and vans, not a walk-in retail counter, and essentially all of our work is done mobile. Call before coming by. In almost every case it is faster and easier for us to come to you instead.' },
  ],
},
...
<p>Canyon Boulevard turning into CO-119 heading up toward Nederland is a different mechanism entirely. Here the material comes off the canyon walls instead of the road surface. Rockfall and scree accumulate on the narrow shoulders and get redistributed by traffic. The tight lanes mean you often pass oncoming vehicles at close quarters with no room to move over, and the damage tends to be higher-energy than a straightforward highway chip.</p>

<h2>Wind</h2>
<p>Boulder's downslope windstorms are a genuine local hazard and a real cause of glass damage. When conditions line up over the Front Range, gusts coming off the foothills routinely reach speeds that lift gravel, strip branches and send construction material across parking lots. Damage from a windstorm frequently shows up as multiple small impacts across a windshield instead of one clean chip, and a cluster of breaks in one area is often past the point where repair is sensible.</p>
...
<div class="callout">
<p><b>Access is the thing to plan in Boulder.</b> Downtown and the Hill mean narrow alleys, permit parking and loading windows. Give us a rough idea of where the vehicle sits and when it is reachable, and we will schedule around the restriction instead of arriving into one.</p>
</div>

<h2>Where we work in the county</h2>
<p>Boulder proper including downtown, the Hill, Table Mesa, North Boulder and Gunbarrel, plus Louisville, Lafayette, Superior, Erie, Niwot and Longmont. We will go up the canyons to Nederland, Jamestown and Ward, though for those we ask for a bit more scheduling flexibility, because the drive is slow and the weather changes the plan more often than it does on the flat.</p>
`,
  faq: [
...
<p>The practical implication: if a cab window breaks in the middle of harvest you want it reported immediately, not at the end of the week, because the sourcing is the long pole. Send us the make, model and serial plate and we will start chasing the part the same day.</p>

<div class="callout">
<p><b>Hail is the big one here.</b> Weld County sits in one of the most hail-exposed areas in the United States, and severe storms regularly damage vehicles and equipment across wide areas in a single afternoon. After an event, glass supply becomes the bottleneck nationally, so reporting the claim quickly and getting into the queue early makes a real difference.</p>
</div>
...

{
  ...
  heroPoints: [
    ...
    'Southernmost point of our corridor, with no travel charge',
  ],
...
<p>Castle Rock is at roughly 6,200 feet, up on the Palmer Divide, the ridge separating the Denver and Colorado Springs basins. That elevation changes what happens to glass, and people who move up from lower down the corridor notice it.</p>
<h3>Upslope storms</h3>
<p>The Divide forces moist air upward, so Castle Rock gets snow when Denver gets rain and why summer afternoon storms here build fast and hit hard. Hail off these storms is often small but comes in high volume, which produces scattered impact damage across a windshield instead of a single clean break.</p>
<h3>Wind</h3>
